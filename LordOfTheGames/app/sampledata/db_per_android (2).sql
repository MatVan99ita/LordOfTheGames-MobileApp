CREATE TABLE IF NOT EXISTS `achievement` (
  `achievement_id` int(11) PRIMARY KEY,
  ...
  FOREIGN KEY (`game_ref`) REFERENCES `game` (`game_id`)
);
...
CREATE TABLE IF NOT EXISTS `comment` (
  ...
  FOREIGN KEY (`discussion_ref`) REFERENCES  `discussion` (`discussion_id`)
);

CREATE TABLE IF NOT EXISTS `discussion` (
  `discussion_id` int(11) PRIMARY KEY,
  `title` text NOT NULL,
...
CREATE TABLE IF NOT EXISTS `gamecategory` (
  `game_ref` int(11),
  `category_ref` int(11),
  PRIMARY KEY (`game_ref`, `category_ref`),
  FOREIGN KEY (`game_ref`) REFERENCES `game` (`game_id`),
...
CREATE TABLE IF NOT EXISTS `gameplatform` (
  `platform_ref` int(11),
  `game_ref` int(11),
  PRIMARY KEY (`game_ref`, `platform_ref`),
  FOREIGN KEY (`game_ref`) REFERENCES `game` (`game_id`),
  FOREIGN KEY (`platform_ref`) REFERENCES `platform` (`platform_id`)
);
...
CREATE TABLE IF NOT EXISTS `platform` (
  `platform_id` int(11) PRIMARY KEY,
  `nome` text NOT NULL,
  `icona` text NOT NULL
);
